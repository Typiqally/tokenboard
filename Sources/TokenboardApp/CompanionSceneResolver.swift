...
struct CompanionResolvedActorField: Equatable, Sendable {
    ...
    init(field: CompanionActorField, seed: UInt64) {
        ...
        self.constants = (0..<field.count).map { index in
            ...
            let sprite = field.sprites.isEmpty
                ? nil
                : field.sprites[Int(rng.next() % UInt64(field.sprites.count))]
            return Constants(
                phase: phase,
                tempo: tempo,
                height: height,
    ...
    func append(at elapsed: Double, into actors: inout [CompanionActor]) {
        for constant in constants {
            ...
            let spriteFrame = constant.sprite.map { sprite in
                guard sprite.frameCount > 1, sprite.framesPerSecond > 0 else { return 0 }
                let cycle = CompanionMath.fraction(
                    elapsed * sprite.framesPerSecond / Double(sprite.frameCount)
                        + constant.phase
                )
