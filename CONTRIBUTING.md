...
`Scripts/audit-local-usage.sh` is an optional, read-only diagnostic. It requires `jq` and `sqlite3`; the script does not install either. A user must first grant/select the real roots in Tokenboard, use Reveal Local Data to identify the exact ledger, and then explicitly invoke the audit with all three variables:
...
The script never mutates source logs or the ledger. It keeps parser bookkeeping in a private temporary directory, prints only provider/model/metric aggregates on mismatch, and removes the temporary data on exit. It deliberately refuses to guess roots or run without all three variables. Because it is an independent shell parser, a mismatch is a diagnostic to investigate, not authorization to rewrite either source logs or the Tokenboard ledger.
