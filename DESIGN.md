...
- A native All / Input / Output menu beside the summary period controls token scope. History and Settings share the persisted preference. Scope changes invalidate summary/history queries and chart selections without rescanning logs; stale results cannot replace the current scope. An independent all-token Today snapshot supplies companions and Discord. Pricing coverage diagnostics remain unfiltered.
...
- With a companion enabled, turn the popover's top `350 × 224` into a full-bleed panorama and keep the complete surface at `350 × 656`. The native period, refresh, quit, headline, and API-value label overlay the scene under a restrained top scrim, while the range control starts the solid data surface below it. The world bleeds edge to edge between two hairlines with soft inset shading — a window cut through the popover surface, never a bordered card. A slim 3-point progress line hugs the panorama's bottom edge over a soft scrim, showing how far the journey is toward the next stage; the artwork itself carries no title, stage, token count, or caption. `None` renders no placeholder and leaves the `350 × 560` layout unchanged.

### Companion settings

- General Settings uses a one-click visual shelf for `None`, Pokémon, Forest, Village, Old School RuneScape, Age of Empires II, Minecraft, and Banished. A checkmark and border identify selection without relying on color alone.
...
- Every scene is baked at development time (`Scripts/fetch-companion-assets.sh`, `Scripts/bake-companion-assets.swift`, `Scripts/generate-companion-artwork.swift`) into a Retina panorama plate (`1240 × 836` for source-based worlds, `1240 × 832` for generated pixel worlds), so the popover never stretches a former strip or fetches artwork at runtime. Compact previews bottom-crop the same plate to preserve the subjects' ground line. Official Pokémon artwork renders stand on a painted foreground before Let's Go location vistas; original generated pixel-art Forest and Village plates grow at runtime; equipped Old School RuneScape characters are grounded in full-resolution location scapes; Age of Empires II Definitive Edition uses West European settlement renders with a matched player color across all twelve stages; Minecraft uses biome, structure, and dimension screenshots with canonical player and armor renders; Banished and Frostpunk use developer-hosted settlement and city views. Branded inhabitants are also baked into compact horizontal PNG strips or transparent one-frame sprites from source renders and GIF frames. Scenes never use generated approximations of branded artwork.
- Pokémon rotates among bundled starter families and gathers the family in its final scene. Forest and Village grow continuously: a deterministic per-install layout keeps planting trees and raising buildings as tokens accrue — subjects appear between milestones and mature through sprite levels (saplings become ancient trees; cottages redevelop into high-rises behind the streetfront) — while twelve distinct plates carry the season and time-of-day arc. Old School RuneScape pairs each equipment tier with its canonical location, Age of Empires II grows one West European settlement from a Dark Age camp to an Imperial capital, Minecraft takes one survivor from a plains spawn through the Nether to the End, and Banished follows exiles from their first shelter to a township that survives winter.
...
- Menu-bar silhouettes derive from each theme's transparent subject artwork; Age of Empires II and Banished use drawn settlement glyphs because their scenes are opaque screenshots. Every stage has a readable `18 × 18` template silhouette and no icon is a rectangular photograph.
...
- Always show the exact preview as `Playing Tokenboard`, a daily-usage detail line, today's compact tokens plus estimated focus duration, and the static `View on GitHub` action. The preview has one combined VoiceOver label in spoken order.
- Preview the resolved 64-point square artwork beside the activity text, using the bundled 128px copy of the uploaded still. Match the selected companion’s theme, daily starter family, and stage, with fixed scenery and a shared seed. Keep the normal Tokenboard icon for `None` or unpublished artwork; explain unavailable companion artwork inline. Include the image tooltip in VoiceOver and confirmation copy.
- On first enable, show a native confirmation alert containing that exact preview and the disclosure that Discord may show it on the user's profile, friend lists, and server member lists and may attach the public GitHub link. Cancel leaves the preference off.
...
| Banished | A few exiles become a working town. Authentic citizens patrol open fields or run reversible errands along a stage's real plaza, dock, bridge, market path, or village road, scaled to the depth of that corridor so nobody walks across a roof. Population grows without crowding the band. Smoke rises from the settlement, autumn leaves cross its market years, a weather front darkens the plate, and the hard-winter stages carry snow. | The year turning |

- The whole scene is one `Canvas`: the background plate paints first; transparent subjects and inhabitants then share a far-to-near order based on their ground line; perched birds paint after the surface they sit on; atmosphere and wash cross the finished world. A lit window therefore cannot float over the building in front of it, people can pass both behind and in front of a subject, and weather still passes over everyone underneath it.
- Every inhabitant keeps a body plan — biped, quadruped, flier — and a route: a patrol with rests, a gatherer's errand, a tile-per-tick pace, a wander that lingers at the ends of its beat, or a flight that lands somewhere the layout actually put a perch. Forest and Village draw those plans as original Tokenboard pixel art. Pokémon, Old School RuneScape, Age of Empires II, Minecraft, and Banished attach recognizable source sprites from their own games; source GIFs become equal-cell PNG strips and source stills become one-frame strips. Routes still own position, facing, attention, and pace, while the strip owns visual identity and source animation. Pixel worlds snap their people to the plate's own art-pixel grid and give them a hard contact shadow instead of a blur.
...
- Work Patterns is insight-first without becoming a card dashboard. For 7D/30D/90D it leads with three stable divider-separated observations—rhythm, block shape, and AI interaction—then retains estimated focus metrics, selectable daily activity, schedule evidence, and a 7×24 Focus time/Consistency/Token volume heatmap. Today omits recurring conclusions and uses a 24-hour estimated-focus timeline.
- Rhythm observations require three active days; middle-50% start/end ranges require four. Block-shape and aggregate tool-mix observations require five focus blocks. Provider-filtered history substitutes median within-block interaction cadence after five recorded gaps. Unqualified slots remain visible as quiet learning states.
- Block profiles use 5–10, 15–25, 30–55, and 60+ minute bands. Sustained-focus share means estimated minutes in blocks of at least 30 minutes. Estimate makeup separates provider-deduplicated activity-backed slices from minutes bridged by the focus-block rule.
- One activity slice is one provider-only five-minute local bucket containing additive usage. Multiple providers in the same slice count once. Slices up to 15 minutes apart form one estimated focus block; larger gaps start another. Informational reasoning subsets never create activity. The surface says “estimated focus,” never “hours worked” or “productivity.”
- When the selected range predates reliable activity-slice coverage, show the coverage start and exclude earlier totals from focus metrics while retaining them in Usage.
...
- `UsageQueryService.history` queries the widest requested local range once, reuses one validated pricing index, and produces deterministic snapshots. `AppModel` refreshes and caches Today, 7D, 30D, and 90D snapshots immediately after authoritative imports and pricing changes; incremental ingestion refreshes at most once every 30 seconds, with one trailing refresh when changes arrive inside that window.
- `FSEventWatcher` uses native file events as the fast path. As a bounded fallback for silently missed appends, every five seconds it reads metadata for at most 64 recent JSONL files, reserving capacity fairly across the granted roots. A complete read-only metadata inventory runs every 15 minutes, so a missed native event for a file outside the recent set can take up to 15 minutes to surface. Neither reconciliation path reads source contents.
- `SQLiteLedger` stores provider-only five-minute activity slices alongside content-safe token aggregates. `WorkPatternCalculator` derives conservative focus blocks, recurring windows, schedule ranges, block profiles, estimate makeup, interaction cadence, and provider mix on demand; `WorkPatternView` renders them without storing sessions, insights, or productivity scores.
- `DiscordPresenceCoordinator` owns opt-in state and serializes lifecycle reconciliation, coalescing changes received during connection or publication. `DiscordCompanionArtwork` maps the existing companion presentation to a finite public still; `DiscordArtworkResources` resolves the bundled preview and application-specific publication list. The debug-only exporter reuses the scene renderer’s square layout before app startup, without accessing user data. `DiscordIPCClient` implements Discord's framed local Unix-socket protocol directly, verifies a same-user socket and peer, and serializes only the allowlisted preview fields.
...
- `CompanionSceneMotion` is the pure motion vocabulary — signatures, particle fields, shadow bands, glows, washes, subject motion, and window lighting — with no AppKit or SwiftUI in it. `CompanionSceneActors` is the equally pure population layer: body plans, routes, sprite-frame selection, and the attention a world's subjects answer to; `CompanionActorSpriteCatalog` is the single bundle-relative catalog for branded inhabitants. `CompanionSceneResolver` freezes each plan's seeded constants once per composition and advances frames from them. `CompanionSceneDirection` holds the per-world, per-stage art direction as data. `CompanionSceneView` hosts the scene's timeline, `CompanionSceneComposition` composes a scene once per strip or panorama layout, and `CompanionSceneCanvas` sequences every frame's passes in one `Canvas` over the per-concern renderers — `CompanionPlateRenderer`, `CompanionSubjectRenderer`, `CompanionActorRenderer`, `CompanionParticleRenderer`, and `CompanionAtmosphereRenderer` — while `CompanionSceneDepth` establishes their shared painter order. `CompanionActorFigure` and `CompanionParticleGeometry` supply pure shape math through `CompanionSceneRendering`'s primitive vocabulary. `CompanionPanorama` frames the popover world, `CompanionStrip` preserves compact preview composition, and `CompanionAssetImageStore` caches the bundled artwork; `CompanionWindowMap` recovers each village sprite's own window grid from its pixels; `CompanionSceneVisibility` gates motion on the host window really being on screen; `CompanionMenuIcon` renders the menu silhouettes.
...
- Do not add streaks, rewards, restart/prestige loops, runtime companion uploads, user artwork configuration files, or runtime asset downloads. Discord stills are exported and uploaded during development; the runtime sends only verified asset keys over local IPC.
