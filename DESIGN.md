...
- Work Patterns uses divider-separated metrics rather than dashboard cards: estimated focus time, focus-block count and average length, active-day averages, selectable daily activity, a 7×24 Token volume/Consistency heatmap, volume and recurrence peaks, typical first/last activity, longest focus block, and busiest date. Today substitutes a 24-hour token timeline for recurring weekly patterns.
...
- `SQLiteLedger` stores provider-only five-minute activity slices alongside content-safe token aggregates. `WorkPatternCalculator` derives conservative focus blocks on demand; `WorkPatternView` renders them without storing session or productivity scores.
