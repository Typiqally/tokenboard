...
- Discord Activity is off by default. On first enable, Tokenboard shows the exact public activity and explains where Discord may display it. If confirmed, Tokenboard publishes only the app name, today's compact token total, today's estimated focus duration, and one static `View on GitHub` button linking to `https://github.com/Typiqally/tokenboard`. The local RPC protocol also carries Tokenboard's shared public Discord application ID, process ID, and a random per-request nonce to the same-user Discord desktop client's Unix socket. It sends no provider, model, project, path, conversation, cost, activity-slice timestamps, party data, or secrets. Discord controls how that public activity appears and may process it under Discord's own privacy terms. Disabling the toggle or quitting Tokenboard clears the activity when the client is reachable. Tokenboard requires renewed consent when the public metric changes.
...
Committed daily and hourly aggregates and five-minute activity slices deliberately survive source-log deletion, a changed source root, and revoked folder access. That is the cache/ledger behavior that lets Tokenboard remember counts and focus estimates after conversations are cleared. Revocation does not erase prior data.
